...
const NetworkStats = () => { 
  ...
  const mem = useCallback(async () => {
      ...
      try {
          setLoading(true);
          const response = await systemService.getNetworkStats(); 
          setMemData(response.data[0]);
         /*
            const curr = response.data[0];
            const now = Date.now(); 
            if (prevStats) {
                ...
                console.log(`⬇ Download: ${rxSpeedMbps.toFixed(2)} Mbps`);
                console.log(`⬆ Upload  : ${txSpeedMbps.toFixed(2)} Mbps`);
            } 
            prevStats = curr;
            prevTime = now;*/
   ...
   useEffect(() => {
        mem();
        const interval = setInterval(mem, 5000);
